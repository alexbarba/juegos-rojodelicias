...
class Game extends React.Component {
	...
	writeHistory(state) {
		...
		this.setState(() => {
			return {
				history: [...this.state.history, { squares: squares, score: score }]
			};
		});
	}
		...
		return (
			<div className="game" id="game">
				<div className="game-board" id="game-board">
					...
					<div className="status">
						{this.state.rewinds > 0 ? (
							<span>Rewinds used: {this.state.rewinds}</span>
						) : null}
					</div>
					<div className="game-info">
						<span>
							{this.state.isStarted && !this.state.isMoved
								? "Nothing has moved"
								: null}
						</span>
					</div>
				...
				<div className="game-buttons">
					<ul>
						<button className="veryBoringButton" onClick={this.reset}>
							New game
						</button>
						<button className="veryBoringButton" onClick={this.rewind}>
							Rewind
						</button>
					</ul>
				</div>
